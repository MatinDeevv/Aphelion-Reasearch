...
class DatasetConfig:
    ...
    persistent_workers: bool = False  # Keep worker processes alive between batches (num_workers>0)
    prefetch_factor: int = 2          # Batches to prefetch per worker (num_workers>0)
...
def build_dataset_from_dataframe(
    ...
    bypassing the DataFrame→list[dict]→DataFrame roundtrip.

    Args:
        df: DataFrame with feature columns (one row per bar, chronological).
            Must contain all CONTINUOUS_FEATURES and CATEGORICAL_FEATURES columns
            that are available; missing columns default to 0.
        close_prices: Array of close prices corresponding to each bar.
        config: Dataset configuration.
    ...
    # Extract continuous features — vectorized, no dict conversion
    cont_matrix = np.zeros((n_bars, n_cont), dtype=np.float32)
    for i, key in enumerate(CONTINUOUS_FEATURES):
    ...
    # Normalize continuous features (z-score, fit on train split only)
    n_usable = n_bars - max_horizon
    train_end = int(n_usable * (1 - config.val_split - config.test_split))
...
def create_dataloaders(
    ...
    extra_kwargs: dict = {}
    if use_workers:
        extra_kwargs["persistent_workers"] = config.persistent_workers
        extra_kwargs["prefetch_factor"] = config.prefetch_factor
